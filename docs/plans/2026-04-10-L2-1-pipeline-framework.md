...
## Dependency Graph (remaining tasks, Tasks 1–8 complete)

> Generated by `parallel-planning` after Tasks 1–8 landed (HEAD `bd3b9c5`). This section covers only the dispatch units still open. Tasks 10 and 11 are each split into their plan-native cycle halves (T10a/T10b, T11a/T11b) because their two cycles own disjoint file sets and can therefore be dispatched independently. Task 13's three cycles are **not** split — they all own `api/errors.py` + `tests/integration/test_error_handling.py`.

### Dispatch units and file ownership

| Task | Depends On | Blocks | Files Owned (exclusive) | Shared Files |
|------|-----------|--------|-------------------------|--------------|
| T9 (within_movement_trend) | — | T10b | `pipeline/stages/within_movement_trend.py`, `tests/unit/pipeline/test_within_movement_trend.py` | — |
| T10a (StageRegistry) | — | T10b | `pipeline/registry.py`, `tests/unit/pipeline/test_registry.py` | — |
| T10b (orchestrator rewrite) | T9, T10a | T11b | `pipeline/orchestrator.py` (replace stub), `tests/unit/pipeline/test_orchestrator.py` | — |
| T11a (storage artifacts) | — | T11b | `pipeline/storage.py` (extend), `tests/unit/test_storage_artifacts.py` | — |
| T14a (synthetic fixture) | — | T11b, T12, T13, T14b | `tests/fixtures/__init__.py`, `tests/fixtures/synthetic_overhead_squat.py` | — |
| T11b (POST /sessions wiring) | T10b, T11a, T14a | T12, T13 | `api/routes/sessions.py` (replace handler), `tests/integration/test_session_pipeline.py` | — |
| T12 (GET report endpoint) | T11b, T14a | T13, T14b | `api/routes/reports.py`, `tests/integration/test_report_endpoint.py` | `api/main.py` (owner) |
| T13 (exception handlers, 3 cycles) | T11b, T12, T14a | T14b | `api/errors.py`, `tests/integration/test_error_handling.py` | `api/main.py` (T12 owns) |
| T14b (end-to-end test) | T11b, T12, T13, T14a | T15 | `tests/integration/test_e2e_overhead_squat.py` | — |
| T15 (final validation) | all above | — | — (validation only, no files) | — |

### Shared files (force serialization)

- **`api/main.py`** — T12 adds `app.include_router(reports.router)`; T13 adds `register_exception_handlers(app)`. Both touch `create_app()`. T12 is the logical owner (it is the first to modify the scaffold). T13 must run **after** T12 to avoid both agents racing on the same file. This serialization is already the plan's execution order.

### Dependency rationale

- **T10b → T9** — `orchestrator.py` imports `run_within_movement_trend` from `pipeline/stages/within_movement_trend.py` (plan line 1820).
- **T10b → T10a** — `orchestrator.py` imports `StageRegistry` from `pipeline/registry.py` (plan line 1804).
- **T11b → T10b** — `api/routes/sessions.py` imports `run_pipeline` from `pipeline/orchestrator.py`; the stub returns a dict and cannot satisfy the test's artifact assertions.
- **T11b → T11a** — the new POST handler calls `storage.save_artifacts(session_id, artifacts)`; method does not exist until T11a lands.
- **T11b → T14a** — `tests/integration/test_session_pipeline.py` imports `build_overhead_squat_payload` from the fixture module.
- **T12 → T11b** — `test_report_endpoint.py` POSTs a session expecting the pipeline to run and persist artifacts; without T11b the POST would not produce artifacts and the GET would 404. (The prior session's pre-analysis missed this edge and placed T12 in parallel with T10 — do not trust that table.)
- **T12 → T14a** — `test_report_endpoint.py` imports `build_overhead_squat_payload`.
- **T13 → T11b** — `test_error_handling.py` POSTs a quality-gate-rejecting payload expecting the 422 handler to fire; requires the POST wiring to exist.
- **T13 → T12** — file conflict on `api/main.py` (serialization, not logical dep).
- **T13 → T14a** — fixture import.
- **T14b → T11b, T12, T13, T14a** — the e2e test exercises the full POST → GET round trip with error handling in place.
- **T15 → all** — final validation runs the entire suite and lints.

### Execution waves

```
Wave A (parallel, 4-wide):   T9, T10a, T11a, T14a      ← all roots, no cross-deps
Wave B (sequential):          T10b                      ← depends on T9 + T10a
Wave C (sequential):          T11b                      ← depends on T10b + T11a + T14a
Wave D (sequential):          T12                       ← depends on T11b + T14a
Wave E (sequential):          T13                       ← depends on T12 (main.py conflict) + T11b
Wave F (sequential):          T14b                      ← depends on T11b + T12 + T13
Wave G (sequential):          T15                       ← depends on everything
```

**Critical path:** T10a → T10b → T11b → T12 → T13 → T14b → T15 (7 waves)
**Max width:** 4 at Wave A, 1 at Waves B–G
**Parallelism factor:** 10 dispatch units in 7 waves = 1.43× (modest — the chain from T11b → T14b is inherently sequential)

### Execution strategy

> **For Claude:** Use `parallel-plan-executor` to execute the remaining waves. The repo's `feedback_no_subagent_worktrees.md` rule forbids `isolation: "worktree"` on subagents, and the sequential-in-main-tree rule forbids concurrent dispatch in the same git tree due to the `.git/index.lock` race. **This leaves only two viable strategies for Wave A's 4-wide parallelism:**
>
> 1. **Sequential dispatch in main tree** (safe default, matches repo memory). 4 sequential task-executor runs for Wave A, then each subsequent wave serially. Zero risk, ~4× runtime on Wave A vs theoretical parallel. **Recommended.**
> 2. **Worktree-isolated parallel dispatch** (experimental, overrides the memory ban). Requires user authorization per-task. Not recommended — the historical ban reason (commit permission failures) has not been re-tested.
>
> **Decision point:** The user must pick strategy 1 or 2 before Wave A dispatches. Default to strategy 1 unless explicitly told otherwise.

**Wave-by-wave dispatch:**

- **Wave A** — T9, T10a, T11a, T14a (sequential, 4 commits total)
- **Wave B** — T10b (orchestrator, 1 commit)
- **Wave C** — T11b (POST wiring, 1 commit; note Plan 1's original task lists 2 commits inside T11 but that folds T11a+T11b together — we've split them)
- **Wave D** — T12 (report endpoint, 1 commit)
- **Wave E** — T13 (exception handlers, **3 commits**: QualityGateError → 422, StageError → 500, generic PipelineError → 500)
- **Wave F** — T14b (e2e test, 1 commit)
- **Wave G** — T15 (final validation, 0–1 commits depending on lint/format fixes)

Expected commit count: 4 (Wave A) + 1 + 1 + 1 + 3 + 1 + 0 = **11 commits** to finish Plan 1, landing on `main`.
