...
UPPER_LIMB_LANDMARK_INDICES = (11, 12, 13, 14, 15, 16)
MIN_UPPER_LIMB_VISIBILITY = 0.6
...
def run_quality_gate(ctx: StageContext) -> SessionQualityReport:
    ...
    if ctx.movement_type == "bicep_curl":
        upper_vis: list[float] = []
        for frame in ctx.session.frames:
            for idx in UPPER_LIMB_LANDMARK_INDICES:
                if idx < len(frame.landmarks):
                    upper_vis.append(frame.landmarks[idx].visibility)
        upper_mean = sum(upper_vis) / len(upper_vis) if upper_vis else 0.0
        metrics["upper_limb_visibility"] = upper_mean
        if upper_mean < MIN_UPPER_LIMB_VISIBILITY:
            issues.append(
                QualityIssue(
                    code="low_upper_limb_visibility",
                    detail=f"upper-limb avg visibility {upper_mean:.2f} < {MIN_UPPER_LIMB_VISIBILITY:.2f}",
                )
            )
